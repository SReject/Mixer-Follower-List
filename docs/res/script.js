(function () {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState == XMLHttpRequest.DONE) {
                    if (xhr.status === 429 || xhr.getResponseHeader('x-ratelimit-remaining') === "0") {
                        callback("You have been ratelimited by mixer", xhr, state);

                    } else if (xhr.status === 404) {
                        callback("Channel does not exist", xhr, state);

                    } else if (xhr.status !== 200) {
                        callback("mixer reported an error: " + xhr.status, xhr, state);
    ...
    function validateChannel(chan, callback) {
        ...
        request("https://mixer.com/api/v1/channels/" + chan, function (err, xhr, state) {
    ...
    (function () {
        ...
        validateChannel(chan, function (err, xhr, state) {
            ...
            try {
                ...
                log.error("mixer returned invalid response");
                throw new Error("mixer returned invalid response");
            }
            ...
            if (xhr.getResponseHeader("x-ratelimit-remaining") !== null && xhr.getResponseHeader("x-ratelimit-remaining") <= 1) {
                log.error("You do not have a large enough mixer-api rate limit buffer to continue");
                throw new Error("You do not have a large enough mixer-api rate limit buffer to continue: " + xhr.getResponseHeader("x-ratelimit-remaining"));
            }
            ...
            function processFollowers(xhr, page) {
                ...
                try {
                    ...
                    log.error("mixer returned an invalid response");
                    return false;
                }
            ...
            request("https://mixer.com/api/v1/channels/" + res.id + "/follow?limit=100&page=0", function (err, xhr) {
                if (err) {
                    log.error(err);
                    throw new Error(err);
                }
                if (xhr.getResponseHeader('x-total-count') == null) {
                    log.error("mixer failed to report total number of followers");
                    throw new Error("mixer failed to report total number of followers");
                }
                if (xhr.getResponseHeader('x-ratelimit-remaining') == null) {
                    log.error("mixer failed to report the number of requests left in your ratelimit buffer");
                    throw new Error("mixer failed to report the number of requests left in your ratelimit buffer");
                }
                ...
                for (var pageIndex = 1, req; pageIndex < pageCount; pageIndex += 1) {
                    ...
                    if (pageIndex === 1) {
                        req = request("https://mixer.com/api/v1/channels/" + res.id + "/follow?limit=100&page=" + pageIndex, callback, pageIndex);
                    } else {
                        req.queue("https://mixer.com/api/v1/channels/" + res.id + "/follow?limit=100&page=" + pageIndex, callback, pageIndex);
                    }
                }
            });
        });
